...
def get_model_with_precision(config, tokenizer):
    ...
    attention_backend_env = os.environ.get('ATTENTION_BACKEND', None)
    if attention_backend_env is not None:
        from megatron.core.transformer.enums import AttnBackend
        base_llama_config.attention_backend = AttnBackend[attention_backend_env]
    if config.model.activations_checkpoint_granularity is not None:
        base_llama_config.recompute_granularity = config.model.activations_checkpoint_granularity
    if config.model.activations_checkpoint_method is not None:
        base_llama_config.recompute_method = config.model.activations_checkpoint_method
    if config.model.activations_checkpoint_num_layers is not None:
        base_llama_config.recompute_num_layers = config.model.activations_checkpoint_num_layers
